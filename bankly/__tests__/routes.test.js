...
//BUG 1 TEST
test("Does not authenticate a user with an invalid token", async function () {
...
test("Throws an error when 'items' is not an object", function () {
  expect(() =>
    sqlForPartialUpdate("users", "not-an-object", "id", 1)
  ).toThrow();
});

//BUG 5 is a syntax error of a duplicate command - no test needed
//BUG 6 is missing a .config to add variables to the required dotenv module - no test needed
